...
st.markdown("""Dados de funcionamento Honda Civic 2003 - ODBII & app Torque Pro  
...
* Arquivo exportado para formato CSV pelo proprio app.
 Os dados obtidos são:
	...
	* G_x	--> Valor de leitura do aceletrômetro do celular, eixo X
	* G_y	--> Valor de leitura do aceletrômetro do celular, eixo y
	* G_z	--> Valor de leitura do aceletrômetro do celular, eixo z
	* G_calibrated	--> item a ser desvendado futuramente
	* GPS_Altitude(m)	--> altitude obtida do GPS do telefone
	* Engine Load	--> valor de 0 a 100% da utilização da força do motor
    * Average CO2 (g/km)	--> quantidade média de emissão de gás carbônico durante a medição atual e a anterior
	* Trip Distance(km)	--> distância percorrida desde o início da jornada
	* GPS Accuracy(m)	--> erro aproximado da localização do GPs
	* km/l(Instant)	--> consumo instantâneo em km/l
	* GPS Latitude	--> coordenada de GPS obtido do telefone celular
	* GPS Longitude	--> coordenada de GPS obtido do telefone celular
	* Throttle Position	--> valor de 0 a 100% referente à posição do pedal do acelerador:
		* 0%: pedal em repouso
		* 100%: pedal totalmente pressionado
	* Intake Manifold Pressure(psi)	--> pressão do ar na admissão de ar do motor
	...
	* Intake Air Temperature	--> temperatura do ar na admissão de ar do motor
